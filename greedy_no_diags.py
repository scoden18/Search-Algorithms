#Doesn't always give the shortest path, but gets pretty close.
#But the actual runtime is way less than the non-directionalized ones.
#Runtime is usually worse than diagonal directionalized because that one can do a better job
#of pointing right at the target since it has more motion options.
#But with fewer obstructions this one tends to be *slightly* faster.
...
def searchnear(tile):
    ...
    while tile != target_tile:
        ...
        try:
            ...
            if (3*math.pi/4 < math.atan2(dir_vector[1],dir_vector[0]) <= math.pi) or (-math.pi <= math.atan2(dir_vector[1],dir_vector[0]) < -3*math.pi/4):
                if left in valids:
                    dir_offset = "left"
                    next_tile = left
            elif -math.pi/4 < math.atan2(dir_vector[1],dir_vector[0]) < math.pi/4:
                if right in valids:
                    dir_offset = "right"
                    next_tile = right
            elif -math.pi/4 >= math.atan2(dir_vector[1],dir_vector[0]) >= -3*math.pi/4:
                if up in valids:
                    dir_offset = "up"
                    next_tile = up
            elif math.pi/4 <= math.atan2(dir_vector[1],dir_vector[0]) <= 3*math.pi/4:
            ...
            #If the preferred direction is blocked, instead searches the tile closest to the target
            if not dir_offset:
                next_tile = unsearched[0]

                mindist = math.sqrt((getx1(target_tile[0]) - getx1(next_tile[0]))**2 + (gety1(target_tile[0]) - gety1(next_tile[0]))**2)
            ...
            #If the preferred direction isn't blocked, goes that way.
            else:
                if next_tile == target_tile:
                    target_found = True
                    canvas.itemconfigure(next_tile[0], fill = "#FF00FF")
